...
function Thresh:LoadMenu()
	self.Menu = MenuElement({type = MENU, id = "Thresh", name = "Thresh", leftIcon = "https://i.imgur.com/fvBONAB.jpg"})
	self.Menu:MenuElement({type = MENU, id = "Combo", name = "Combo"})
	self.Menu.Combo:MenuElement({id = "HQengage", name = "HQengage Q For Engage", value = false})
	self.Menu.Combo:MenuElement({id = "KTM", name = "KT E>AA>Q", value = true})
	self.Menu.Combo:MenuElement({id = "WolfCombo", name = "Wolf Combo Q>R>AA>E", value = true})
	self.Menu.Combo:MenuElement({id = "HotKeyChanger", name = "Key Changer", key = 0x5a, toggle = true, value = true})
...
function Thresh:Engage()
	
	local target = (_G.GOS and _G.GOS:GetTarget(850,"AD"))
	if target == nil then return end
	if target == nil or self:IsValidTarget(target,850) == false then return end
		if target.pos:DistanceTo(myHero.pos) >= 650 and self:IsReady(_Q) and self.Menu.Combo.HQengage:Value() and target:GetCollision(Q.width,Q.speed,Q.delay) == 0 then
...
function Thresh:QEintoR()
	local target = (_G.GOS and _G.GOS:GetTarget(850,"AD"))
	if target == nil then return end
	if target == nil or self:IsValidTarget(target,850) == false then return 
	end
...
function Thresh:Prediction(unit)
...
function Thresh:fast(spell, unit, prediction, delay)
...
function Thresh:GetTarget(range)
	local target
...
function Thresh:GetBuffs(unit, ability)
...
function Thresh:CanUseSpell(spell)
...
function Thresh:Draw()
	...
	if (_G.GOS and _G.GOS:GetMode() == "Combo") then
		...
		if self.Menu.Combo.KTM:Value() and self.Menu.Combo.HotKeyChanger:Value() then
			self:Engage()
		elseif self.Menu.Combo.WolfCombo:Value() and self.Menu.Combo.HotKeyChanger:Value() == false then
			self:QEintoR()
		end
	...
	if self.Menu.Combo.WolfCombo:Value() and self.Menu.Combo.HotKeyChanger:Value() == false then
